...
Here we produce the same tracks path plot but this time we same 1 plot per timepoint. Later on we can assemble a stack image (in ImageJ for example) to produce a progression video that shows the tracks development over time.
```{r}
#i = 24

for(i in 1:max(Data$Slice)){
  #print (i)
  Data_plot = Data %>% filter(Slice <= i)
  Tracks_Lines_Centered <- ggplot(Data_plot, aes(x = X_Shift, y = Y_Shift, color=as.factor(Track)))+ 
                          geom_path(aes(group = Track), size = 1)+
                          geom_point(x=0,y=0, col = "red", size = 3)+
                          theme_bw()+
                          xlab('X (um)')+
                          ...
                          xlim(-400, 400)+
                          ylim(-400, 400)
                          #scale_x_continuous(breaks=seq(-500, 500, by = 100)))+
                          #scale_y_continuous(breaks=seq(-500, 500, by = 100))
  Tracks_Path_Centered
  ggsave(filename=paste("Tracks",i,".png",sep=""), height = 7, width = 7, plot=Tracks_Lines_Centered)
}
